...
//
// Modular fork: omit `FieldLabel` when `label` is null/undefined/empty so deposit config
// can hide the title row (matches replacement `TextField`: no icon-only blank header).
    ...
    return (
      <GroupField className="main-group-field">
        ...
        <SubjectAutocompleteDropdown
          {...dropdownProps}
          fieldPath={fieldPath}
          label={!displaySuggestFromField ? labelEl : undefined}
          limitTo={limitTo}
