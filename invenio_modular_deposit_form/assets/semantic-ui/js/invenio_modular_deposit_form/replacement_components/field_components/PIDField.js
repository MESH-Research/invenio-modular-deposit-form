...
//
// KCWorks replacement note (how this differs from stock):
// - Stock source: invenio-rdm-records PID field modules
//   (`PIDFieldCmp`, `RequiredPIDField`, and related components).
// - Why this copy exists: DOI/PID behavior needed local customization while keeping
//   the same public component API (`PIDField`) used by form config.
// - Main differences from stock:
//   1) Error-display logic for the unmanaged identifier input is intentionally
...
//   3) Dependency wiring is local to invenio_modular_deposit_form:
//      - translations use modular-deposit-form namespace,
//      - submit action/context constants are defined locally here to avoid importing
//        the stock app-rdm module internals.
// - Non-goal: this file is not intended to be a general refactor; outside the
//   items above it should track stock behavior as closely as practical.
...
class UnmanagedIdentifierCmp extends Component {
  ...
  render() {
    ...
    // Errors for DOI/PID may be attached to the pid object (fieldPath) or its nested `identifier`.
    const currentError =
      getIn(form.errors, fieldPath, null) ??
      getIn(form.errors, `${fieldPath}.identifier`, null);
    const initialError =
      getIn(form.initialErrors, fieldPath, null) ??
      getIn(form.initialErrors, `${fieldPath}.identifier`, null);

    // Mirror `replacement_components/TextField.js` logic:
    // - show error on first blur (i.e. touched)
    // - OR show error if the current value is the same as the initial value and there was an initialError
    const initialIdentifier =
      getIn(form.initialValues, `${fieldPath}.identifier`, "") ?? "";
    const showError = (currentError && touched) || (localIdentifier === initialIdentifier && !!initialError);

    const inputError = showError ? (currentError ?? initialError) : undefined;
    return (
      <>
        {/* // <Form.Group> */}
