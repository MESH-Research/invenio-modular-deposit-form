 ...
 * When neither `className` nor `id` is set, renders a Fragment so callers (e.g.
 * FormFooterRegion's sticky grid) can own the wrapping element.
 */
const SubsectionsRenderer = ({ subsections = [], className, id, isFormPagesRegion = false }) => {
  const items = subsections.map(
    (
      {
        section,
        component,
