...
import { DepositFormApp } from "./patched_rdm_core/deposit/DepositFormApp";
...
export const RDMDepositForm = ({
  ...
  // generated by frontend here. Type options mirror stock invenio-app-rdm.
  //
  // Creators role: use only `config.vocabularies.creators.role` when non-empty.
  // Do not fall back to `contributors.role` — contributor roles are a different
  // facet; merging them showed creator role UI and illegal values when no creator
  // role vocabulary was configured.
  const creatorRoleVocab = config.vocabularies.creators?.role;
  const creatorsRoleForStore =
    Array.isArray(creatorRoleVocab) && creatorRoleVocab.length > 0 ? creatorRoleVocab : [];
