...
const fieldComponents = {
  abstract: [AbstractComponent, ["metadata.description"]],
  additional_dates: [AdditionalDatesComponent, ["metadata.dates"]],
  ...
  ai: [AIComponent, ["custom_fields.kcr:ai_usage"]],
  ...
  contributors: [ContributorsComponent, ["metadata.contributors"]],
  creators: [CreatorsComponent, ["metadata.creators"]],
  date: [DateComponent, ["metadata.publication_date"]],
  doi: [DoiComponent, ["pids.doi"]],
  funding: [FundingComponent, ["metadata.funding"]],
  isbn: [null, ["custom_fields.imprint:imprint.isbn"]],
  file_upload: [FilesUploadComponent, ["files"]],
  keywords: [KeywordsComponent, ["custom_fields.kcr:user_defined_tags"]],
  language: [LanguagesComponent, ["metadata.languages"]],
  licenses: [LicensesComponent, ["metadata.rights"]],
  metadata_only: [MetadataOnlyComponent, ["access.status"]],
  previously_published: [PreviouslyPublishedComponent, []],
  publisher: [PublisherComponent, ["metadata.publisher"]],
  publication_location: [
    PublicationLocationComponent,
    ["custom_fields.imprint:imprint.place"],
  ],
  related_works: [RelatedWorksComponent, ["metadata.related_identifiers"]],
  resource_type: [ResourceTypeComponent, ["metadata.resource_type"]],
  series: [SeriesComponent, ["custom_fields.kcr:book_series"]],
  subjects: [SubjectsComponent, ["metadata.subjects"]],
  subtitle: [SubtitleComponent, ["metadata.additional_titles"]],
  title: [TitleComponent, ["metadata.title"]],
  total_pages: [TotalPagesComponent, ["custom_fields.imprint:imprint.pages"]],
  volume: [VolumeComponent, ["custom_fields.kcr:volumes"]],
  version: [VersionComponent, ["metadata.version"]],
  // below are composite field components
  access_rights: [AccessRightsComponent, ["access"]],
  ...
  submit_actions: [SubmitActionsComponent, ["access"]],
  thesis_detail: [ThesisDetailsComponent, ["custom_fields.thesis:university"]],
  type_title: [
    TypeTitleComponent,
...
export const RDMDepositForm = ({
  ...
  const [currentTypeFieldsByType, setCurrentTypeFieldsByType] =
    useState(undefined);
  const formPages = {
    1: "Title",
    2: "People",
  ...
  const handleErrorsChange = (errors) => {
    if (errors != {}) {
      ...
      for (let p of Object.keys(formPages)) {
        ...
        let pageFields = commonFields[p];
        if (currentTypeFieldsByType) {
          if (!!currentTypeFieldsByType[p]) {
            pageFields = pageFields.concat(currentTypeFieldsByType[p]);
          }
        }
        ...
        let pageMetaFields = pageFields.reduce((accum, curr) => {
          accum = accum.concat(fieldComponents[curr][1]);
          return accum;
        }, []);
                    ...
                    currentFormPage === pageNum && (
                      <div key={index}>
                        ...
                        >
                          ...
                          !!currentTypeFieldsByType[pageNum]
                            ? currentTypeFieldsByType[pageNum]?.map(
                                (component_label, index) => {
                                  const MyField =
                                    fieldComponents[component_label][0];
                                  return (
                                    <MyField
                                      key={index}
                                      config={config}
                                      ...
                                      labelMods={
                                        labelModifications[currentResourceType]
                                      }
                                      currentResourceType={currentResourceType}
                                    />
                                  );
